...
src_url=http://mirrors.svipc.com/Autoscripts/fpm-race-condition.patch && Download_src
...
EOF
    ...
    [ -d "/run/shm" -a ! -e "/dev/shm" ] && sed -i 's@/dev/shm@/run/shm@' $php_install_dir/etc/php-fpm.conf $Autoscripts_dir/vhost.sh $Autoscripts_dir/config/nginx.conf 

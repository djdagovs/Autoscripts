...
for D in `cat ./options.conf | grep dir= | grep -v Autoscripts | grep -v backup_dir | awk -F'=' '{print $2}' | sort | uniq`
do
    [ -e "$D" ] && rm -rf $D
done

sed -i 's@^Autoscripts_dir=.*@Autoscripts_dir=@' ./options.conf
...
echo 
echo "${CWARNING}You will uninstall Autoscripts, Please backup your configure files and DB data! ${CEND}"
echo 
...
do
    echo
    read -p "Do you want to uninstall Autoscripts? [y/n]: " uninstall_yn

...
printf "
#######################################################################
#      Autoscripts for CentOS/RadHat 5+ Debian 6+ and Ubuntu 12+      #
#       For more information please visit http://www.svipc.com        #
#######################################################################
"

# get pwd
sed -i "s@^Autoscripts_dir.*@Autoscripts_dir=`pwd`@" ./options.conf
...
. ./include/memory.sh
if [ "$OS" == 'CentOS' ];then
    . include/init_CentOS.sh 2>&1 | tee $Autoscripts_dir/install.log
    [ -n "`gcc --version | head -n1 | grep '4\.1\.'`" ] && export CC="gcc44" CXX="g++44"
elif [ "$OS" == 'Debian' ];then
    . include/init_Debian.sh 2>&1 | tee $Autoscripts_dir/install.log
elif [ "$OS" == 'Ubuntu' ];then
    . include/init_Ubuntu.sh 2>&1 | tee $Autoscripts_dir/install.log
fi

# jemalloc or tcmalloc
if [ "$je_tc_malloc_yn" == 'y' -a "$je_tc_malloc" == '1' -a ! -e "/usr/local/lib/libjemalloc.so" ];then
    . include/jemalloc.sh
    Install_jemalloc | tee -a $Autoscripts_dir/install.log
elif [ "$je_tc_malloc_yn" == 'y' -a "$je_tc_malloc" == '2' -a ! -e "/usr/local/lib/libtcmalloc.so" ];then
    . include/tcmalloc.sh
    Install_tcmalloc | tee -a $Autoscripts_dir/install.log
fi

# Database
if [ "$DB_version" == '1' ];then
    . include/mysql-5.6.sh 
    Install_MySQL-5-6 2>&1 | tee -a $Autoscripts_dir/install.log 
elif [ "$DB_version" == '2' ];then
    . include/mysql-5.5.sh
    Install_MySQL-5-5 2>&1 | tee -a $Autoscripts_dir/install.log
elif [ "$DB_version" == '3' ];then
    . include/mariadb-10.0.sh
    Install_MariaDB-10-0 2>&1 | tee -a $Autoscripts_dir/install.log 
elif [ "$DB_version" == '4' ];then
    . include/mariadb-5.5.sh
    Install_MariaDB-5-5 2>&1 | tee -a $Autoscripts_dir/install.log 
elif [ "$DB_version" == '5' ];then
    . include/percona-5.6.sh
    Install_Percona-5-6 2>&1 | tee -a $Autoscripts_dir/install.log
elif [ "$DB_version" == '6' ];then
    . include/percona-5.5.sh 
    Install_Percona-5-5 2>&1 | tee -a $Autoscripts_dir/install.log 
fi

# Apache
if [ "$Apache_version" == '1' ];then
    . include/apache-2.4.sh 
    Install_Apache-2-4 2>&1 | tee -a $Autoscripts_dir/install.log
elif [ "$Apache_version" == '2' ];then
    . include/apache-2.2.sh 
    Install_Apache-2-2 2>&1 | tee -a $Autoscripts_dir/install.log
fi

# PHP
if [ "$PHP_version" == '1' ];then
    . include/php-5.3.sh
    Install_PHP-5-3 2>&1 | tee -a $Autoscripts_dir/install.log
elif [ "$PHP_version" == '2' ];then
    . include/php-5.4.sh
    Install_PHP-5-4 2>&1 | tee -a $Autoscripts_dir/install.log
elif [ "$PHP_version" == '3' ];then
    . include/php-5.5.sh
    Install_PHP-5-5 2>&1 | tee -a $Autoscripts_dir/install.log
elif [ "$PHP_version" == '4' ];then
    . include/php-5.6.sh
    Install_PHP-5-6 2>&1 | tee -a $Autoscripts_dir/install.log
elif [ "$PHP_version" == '5' ];then
    . include/php-7.sh
    Install_PHP-7 2>&1 | tee -a $Autoscripts_dir/install.log
fi

# ImageMagick or GraphicsMagick
if [ "$Magick" == '1' ];then
    . include/ImageMagick.sh
    Install_ImageMagick 2>&1 | tee -a $Autoscripts_dir/install.log
elif [ "$Magick" == '2' ];then
    . include/GraphicsMagick.sh
    Install_GraphicsMagick 2>&1 | tee -a $Autoscripts_dir/install.log
fi

# ionCube
if [ "$ionCube_yn" == 'y' ];then
    . include/ioncube.sh
    Install_ionCube 2>&1 | tee -a $Autoscripts_dir/install.log
fi

# PHP opcode cache
if [ "$PHP_cache" == '1' ] && [ "$PHP_version" == '1' -o "$PHP_version" == '2' ];then
    . include/zendopcache.sh
    Install_ZendOPcache 2>&1 | tee -a $Autoscripts_dir/install.log
elif [ "$PHP_cache" == '2' ];then
    . include/xcache.sh 
    Install_XCache 2>&1 | tee -a $Autoscripts_dir/install.log
elif [ "$PHP_cache" == '3' ];then
    . include/apcu.sh
    Install_APCU 2>&1 | tee -a $Autoscripts_dir/install.log
elif [ "$PHP_cache" == '4' -a "$PHP_version" == '2' ];then
    . include/eaccelerator-1.0-dev.sh
    Install_eAccelerator-1-0-dev 2>&1 | tee -a $Autoscripts_dir/install.log
elif [ "$PHP_cache" == '4' -a "$PHP_version" == '1' ];then
    . include/eaccelerator-0.9.sh
    Install_eAccelerator-0-9 2>&1 | tee -a $Autoscripts_dir/install.log
fi

# ZendGuardLoader (php <= 5.6)
if [ "$ZendGuardLoader_yn" == 'y' ];then
    . include/ZendGuardLoader.sh
    Install_ZendGuardLoader 2>&1 | tee -a $Autoscripts_dir/install.log
fi

# Web server
if [ "$Nginx_version" == '1' ];then
    . include/nginx.sh
    Install_Nginx 2>&1 | tee -a $Autoscripts_dir/install.log
elif [ "$Nginx_version" == '2' ];then
    . include/tengine.sh
    Install_Tengine 2>&1 | tee -a $Autoscripts_dir/install.log
fi

# JDK
if [ "$JDK_version" == '1' ];then
    . include/jdk-1.8.sh
    Install-JDK-1-8 2>&1 | tee -a $Autoscripts_dir/install.log
elif [ "$JDK_version" == '2' ];then
    . include/jdk-1.7.sh
    Install-JDK-1-7 2>&1 | tee -a $Autoscripts_dir/install.log
elif [ "$JDK_version" == '3' ];then
    . include/jdk-1.6.sh
    Install-JDK-1-6 2>&1 | tee -a $Autoscripts_dir/install.log
fi

if [ "$Tomcat_version" == '1' ];then
    . include/tomcat-8.sh
    Install_tomcat-8 2>&1 | tee -a $Autoscripts_dir/install.log
elif [ "$Tomcat_version" == '2' ];then
    . include/tomcat-7.sh
    Install_tomcat-7 2>&1 | tee -a $Autoscripts_dir/install.log
fi

# Pure-FTPd
if [ "$FTP_yn" == 'y' ];then
    . include/pureftpd.sh
    Install_PureFTPd 2>&1 | tee -a $Autoscripts_dir/install.log 
fi

# phpMyAdmin
if [ "$phpMyAdmin_yn" == 'y' ];then
    . include/phpmyadmin.sh
    Install_phpMyAdmin 2>&1 | tee -a $Autoscripts_dir/install.log
fi

# redis
if [ "$redis_yn" == 'y' ];then
    . include/redis.sh
    Install_redis 2>&1 | tee -a $Autoscripts_dir/install.log
fi

# memcached
if [ "$memcached_yn" == 'y' ];then
    . include/memcached.sh
    Install_memcached 2>&1 | tee -a $Autoscripts_dir/install.log
fi

# index example
if [ ! -e "$wwwroot_dir/default/index.html" -a "$Web_yn" == 'y' ];then
    . include/demo.sh
    DEMO 2>&1 | tee -a $Autoscripts_dir/install.log 
fi
...
if [ "$HHVM_yn" == 'y' ];then
    ...
    Install_hhvm_CentOS 2>&1 | tee -a $Autoscripts_dir/install.log 
fi
